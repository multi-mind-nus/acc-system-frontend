  ...
  <section class="space-y-6">
    ...
    <template v-else-if="client || isNew">
      ...
      <template v-if="isNew || tab === 'overview'">
        ...
        <form class="overflow-hidden rounded-xl border bg-card" @submit.prevent="save">
          ...
          <div class="space-y-6 p-6">
            ...
            <fieldset :disabled="saving || !isAdmin" class="space-y-6">
              <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr]">
                <div class="space-y-2"><Label for="legal-name">{{ t('accounts.name') }}</Label><Input id="legal-name" v-model="form.legalName" required pattern=".*\S.*" maxlength="200" class="h-10" /></div>
                <div class="space-y-2"><Label for="client-code">{{ t('accounts.code') }}</Label><Input id="client-code" v-model="form.code" :disabled="!isNew" required pattern="[A-Za-z0-9_-]+" maxlength="50" class="h-10" /></div>
                <div class="space-y-2"><Label for="base-currency">{{ t('accounts.currency') }}</Label><Input id="base-currency" v-model="form.baseCurrency" required pattern="[A-Z]{3}" minlength="3" maxlength="3" class="h-10 uppercase" @input="form.baseCurrency = form.baseCurrency.toUpperCase()" /></div>
              </div>
              <div class="border-t pt-6"><h3 class="text-sm font-semibold">{{ t('accounts.businessFeatures') }}</h3><p class="mt-1.5 text-xs leading-5 text-muted-foreground">{{ t('accounts.featuresHint') }}</p><div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><label v-for="key in featureKeys" :key="key" class="flex cursor-pointer items-center gap-3 text-sm"><input v-model="form.features[key]" type="checkbox" class="size-4 accent-primary focus-visible:outline-ring" />{{ t(`accounts.features.${key}`) }}</label></div></div>
            </fieldset>
          </div>
